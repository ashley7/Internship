...
<div class="d-flex align-items-center gap-3 mb-4">
    ...
    <div class="flex-grow-1">
        <h5 class="mb-0 fw-bold" style="color:#1a5276">Report — {{ $report->date_submitted->format('l, d F Y') }}</h5>
...
<div class="row g-4">
    <div class="col-lg-8">
        ...
        <div class="card mb-4">
            <div class="card-header"><i class="bi bi-clipboard2-pulse me-2"></i>Procedures ({{ $report->reportProcedures->count() }})</div>
                ...
                <table class="table mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <span class="badge {{ $rp->getRoleBadgeClass() }} text-capitalize">
                                    {{ ucfirst($rp->role) }}
                                </span>
                            </td>
                        </tr>
                            ...
                            <div class="d-flex flex-column gap-1">
                                ...
                                <form action="{{ route('student.attachments.destroy', $att) }}" method="POST">
                                    ...
                                    <button type="submit" class="btn btn-sm btn-outline-danger"
                                            onclick="return confirm('Remove?')"><i class="bi bi-trash"></i></button>
                                </form>
                                @endif
                            </div>
                        </div>
        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <form action="{{ route('student.reports.notes.store', $report) }}" method="POST">
                    @csrf
                    <textarea name="note" class="form-control" rows="3" placeholder="Add a note...">{{ old('note') }}</textarea>
                    <button type="submit" class="btn btn-primary btn-sm mt-2"><i class="bi bi-send me-1"></i>Add Note</button>
                </form>
            </div>
        </div>
    </div>
        ...
        <div class="card mb-4">
            ...
            <div class="card-body p-0">
                <table class="table table-sm mb-0" style="font-size:.85rem;">
                    <tr><td class="text-muted ps-3">Date</td><td class="fw-semibold">{{ $report->date_submitted->format('d M Y') }}</td></tr>
                    <tr><td class="text-muted ps-3">Status</td><td><span class="status-badge badge-{{ $report->status }}">{{ ucfirst($report->status) }}</span></td></tr>
                    <tr><td class="text-muted ps-3">Procedures</td><td>{{ $report->reportProcedures->count() }}</td></tr>
                    <tr><td class="text-muted ps-3 pb-3">Attachments</td><td class="pb-3">{{ $report->attachments->count() }}</td></tr>
                </table>
            </div>
        </div>
        <div class="card">
