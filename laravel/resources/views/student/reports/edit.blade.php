...
@php $existingProcedures = $report->reportProcedures; @endphp
...
<div class="row g-4">
    <div class="col-lg-8">
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <input type="text" name="patient_id" class="form-control"
                               value="{{ old('patient_id', $report->patient_id) }}" placeholder="e.g. MUH-2024-001">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">Diagnosis</label>
                        <input type="text" name="diagnosis" class="form-control"
                               value="{{ old('diagnosis', $report->diagnosis) }}" placeholder="e.g. Inguinal hernia">
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card mb-4">
            ...
            <div class="card-body p-4">
                <textarea name="report" class="form-control @error('report') is-invalid @enderror"
                          rows="10" required>{{ old('report', $report->report) }}</textarea>
        ...
        <div class="card mb-4">
            <div class="card-header"><i class="bi bi-paperclip me-2"></i>Existing Attachments</div>
            <div class="card-body">
                <div class="row g-2">
        ...
        <div class="card">
            <div class="card-header"><i class="bi bi-cloud-upload me-2"></i>Add More Attachments</div>
            <div class="card-body p-4">
    ...
    <div class="col-lg-4">
        <div class="card">
            <div class="card-header"><i class="bi bi-floppy me-2"></i>Save</div>
            <div class="card-body">
                <p class="text-muted small mb-3">Only pending reports can be edited.</p>
...
function renderFiles(files) {
    fileList.innerHTML = '';
    [...files].forEach(f => {
        fileList.innerHTML += `<div class="d-flex align-items-center gap-2 p-2 border rounded-3 mb-2 bg-white">
            <i class="bi bi-file-earmark text-primary"></i>
            <div class="flex-grow-1"><div style="font-size:.82rem;font-weight:600;">${f.name}</div>
            <small class="text-muted">${(f.size/1024).toFixed(1)} KB</small></div>
