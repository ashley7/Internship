...
<div class="d-flex align-items-center gap-3 mb-4">
    ...
    <div class="flex-grow-1">
        <h5 class="mb-0 fw-bold" style="color:#1a5276">Report — {{ $report->student->user->name }}</h5>
        <small class="text-muted">Submitted on {{ $report->date_submitted->format('l, d F Y') }}</small>
    </div>
...
<div class="row g-4">
        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <form action="{{ route('supervisor.reports.notes.store', $report) }}" method="POST">
                    @csrf
                    <label class="form-label">Add Feedback / Note</label>
                    <textarea name="note" class="form-control @error('note') is-invalid @enderror" rows="3"
                              placeholder="Write your feedback or comment here...">{{ old('note') }}</textarea>
    ...
    <div class="col-lg-4">
        ...
        <div class="card">
            ...
            <div class="card-body">
                <p class="text-muted small mb-3">Current status: <strong>{{ ucfirst($report->status) }}</strong></p>
